...
const ChatPanel: React.FC = () => {
  ...
  const chatHistoryOpen = useAppStore((s) => s.chatHistoryOpen);
  ...
  // Auto-attach image from command palette "Capturar tela agora"
  useEffect(() => {
    if (pendingImage) {
      // pendingImage is a data URL like "data:image/png;base64,..."
      const mimeMatch = pendingImage.match(/^data:([^;]+);base64,/);
      const mime = mimeMatch?.[1] ?? 'image/png';
      const base64 = pendingImage.replace(/^data:[^;]+;base64,/, '');
  ...
  return (
    <div className="flex flex-col h-full bg-bg">
      ...
      {chatHistoryOpen && (
        <ChatHistoryModal messages={messages} onClear={clearChat} />
      )}
    </div>
  );
};
